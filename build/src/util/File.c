...
#define _util_string_release0(var) ((var == NULL) ? NULL : (var = (util_string_release (var), NULL)))
...
static gpointer _util_string_retain0 (gpointer self) {
	return self ? util_string_retain (self) : NULL;
}
...
gchar** util_file_list (utilFile* self, int* result_length1) {
	...
	if (_tmp7_ != NULL) {
		while (TRUE) {
			...
			s = _tmp12_;
			_tmp13_ = s;
			_tmp14_ = _util_string_retain0 (_tmp13_);
			self->fileList = g_list_append (self->fileList, _tmp14_);
			_util_string_release0 (s);
		}
	}
	...
	_tmp18_ = self->fileList;
	{
		...
		for (s_it = s_collection; s_it != NULL; s_it = s_it->next) {
			...
			utilString* s = NULL;
			_tmp19_ = _util_string_retain0 ((utilString*) s_it->data);
			s = _tmp19_;
			{
				gchar** _tmp20_ = NULL;
	...
	_tmp26_ = (_tmp25_ != NULL) ? _vala_array_dup2 (_tmp25_, _tmp25__length1) : ((gpointer) _tmp25_);
