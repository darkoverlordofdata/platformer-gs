...
class Systems : Object
    ...
    def input(delta: double, ref e: Entity)
        if e.entityType == EntityType.Player
            ...
            if game.inputs[Input.jump]
                timeToFire -= delta
                if timeToFire < 0
                    game.addBullet(e.position.x - 27, e.position.y + 2)
                    game.addBullet(e.position.x + 27, e.position.y + 2)
                    timeToFire = FireRate
            ...
            case e.layer
                ...
                when Layer.EXPLOSION
                    if game.explosions.size > 0
                        ...
                        e.expires = 0.5
                        e.scale = vector2d(exp.scale, exp.scale)
    ...
    def movement(delta: double, ref e: Entity)
    ...
    def expiring(delta: double, ref e: Entity)
                ...
                for var j = 0 to (game.entities.length-1)
                    if game.entities[j].active && game.entities[j].entityType == EntityType.Bullet
                        if game.entities[i].bounds.intersection_rect(game.entities[j].bounds, out result)
                            doCollision(ref game.entities[i], ref game.entities[j])

    def doCollision(ref a:Entity, ref b:Entity)

        if a.entityType == EntityType.Enemy && b.entityType == EntityType.Bullet
            game.addBang(b.position.x, b.position.y)
            b.active = false
            for var i = 0 to 4 do game.addParticle(b.position.x, b.position.y)
            if a.health != null
